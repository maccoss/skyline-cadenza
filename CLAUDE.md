...
- **Push-to-Skyline is a three-stage sequence.** (1) Write a self-contained `.blib` via `BlibAssayWriter`. (2) Register it as a peptide-settings library via `AddSettingsListItem("Libraries", xml)`. (3) Import a peptide-style transition list to populate the target tree. The transition list emits real `Product Charge` values from `FragmentIon.Charge` (no more hardcoded `+1`) and is now correct for the +2 y/b fragments DIA-NN picks for longer tryptic peptides.
- **Live-document transition/peptide settings can't be mutated via the current JSON-RPC interface.** The relevant `SkylineCmd` flags (`--tran-precursor-ion-charges`, etc.) operate on a saved document via `--in` / `--out`, not the in-memory document held by the running Skyline UI we're connected to via the pipe. `SkylineSettingsConfigurator` therefore computes the recommended settings from the assay (precursor / product ion charges, ion types, library-pick top-N, peptide length range) and surfaces them in the status line as advisory guidance the user verifies / applies through the Skyline UI. Closing this loop would need an upstream Skyline RPC like `SetTransitionFilter`; until that exists, the recommendation text is the contract.
